...
abstract class Repository
{
    ...
    /**
     * Returns all records by field = value from this repository's table
     *
     * @param string  $field
     * @param mixed   $value
     * @param integer $limit
    ...
    public function findBy($field, $value, $limit = null)
    {
        if (null === $limit) {
            return $this->db->fetchAll(
                sprintf('SELECT * FROM %s WHERE '.$field.' = ?', $this->getTableName()),
                array($value)
            );
        }

        return $this->db->fetchAll(
            sprintf('SELECT * FROM %s WHERE '.$field.' = ? LIMIT %d', $this->getTableName(), $limit),
            array($value)
        );
    }
